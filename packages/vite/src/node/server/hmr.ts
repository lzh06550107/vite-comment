...
/**
 * 创建了一个 HMR 广播器（HMRBroadcaster），用于管理和广播热模块替换（HMR）相关的消息。
 * 该广播器可以在多个 HMR 通道（HMRChannel）之间广播事件，支持添加通道、发送消息、监听和移除事件等功能
 */
export function createHMRBroadcaster(): HMRBroadcaster {
  // channels 存储当前所有的 HMR 通道。它通过 WeakSet 来追踪已经准备好的通道（readyChannels），确保所有通道都就绪后才触发 'connection' 事件。
  const channels: HMRChannel[] = []
  // 创建了 readyChannels WeakSet 来跟踪已准备好的通道
  const readyChannels = new WeakSet<HMRChannel>()
  const broadcaster: HMRBroadcaster = {
    get channels() {
      return [...channels]
    },
    // 向广播器添加一个新的 HMR 通道。如果通道已经存在（根据通道的 name），则抛出一个错误
    addChannel(channel) {
    ...
    // 给所有的 HMR 通道添加事件监听器。
    // 如果是 'connection' 事件，它会等待所有通道都准备好（通过监听每个通道的 'connection' 事件），只有在所有通道准备好后，才触发传入的监听器。
    // 其他事件会直接传递给所有的通道处理。
    ...
    // 启动所有通道的监听，通常是用于在 HMR 广播器准备好时，启动实际的监听逻辑
    listen() {
      channels.forEach((channel) => channel.listen())
    },
    // 关闭所有的 HMR 通道，并返回一个 Promise，表示所有通道关闭完成后的状态
    close() {
...
/**
 * 用于创建一个 服务器端 HMR 通道（ServerHMRChannel）。
 * 该通道通过事件机制来处理和广播热模块替换（HMR）的信息，尤其适用于在服务器端处理模块更新和与客户端（例如浏览器）进行通信
 */
export function createServerHMRChannel(): ServerHMRChannel {
  const innerEmitter = new EventEmitter() // 用于处理通道内部的事件，像是 HMR 通道连接、关闭等
  const outsideEmitter = new EventEmitter() // 用于处理通道内部的事件，像是 HMR 通道连接、关闭等

  return {
    name: 'ssr',
    // 负责构建一个 HMR 消息（payload），然后通过 outsideEmitter 发射 send 事件，将消息发送到外部
    // 如果第一个参数是字符串，则将其作为 event，第二个参数作为 data。如果参数是对象，则直接作为 payload 发送
    send(...args: any[]) {
    ...
    // 将事件监听器绑定到 innerEmitter 上，监听特定的事件
    on: ((event: string, listener: () => unknown) => {
      innerEmitter.on(event, listener)
    }) as ServerHMRChannel['on'],
    // 移除所有的事件监听器，这样就不会再触发任何事件。它清理了通道中的内部和外部事件监听器
    close() {
      innerEmitter.removeAllListeners()
      outsideEmitter.removeAllListeners()
    },
    // 触发 connection 事件，这可能表示一个新的连接已经建立，可以开始进行通信
    listen() {
      innerEmitter.emit('connection')
    },
    // 公开了 innerEmitter 和 outsideEmitter，允许外部代码访问这些事件发射器，以便直接进行事件处理或发送
    api: {
      innerEmitter,
      outsideEmitter,
    },
