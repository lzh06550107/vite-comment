...
/**
 * 用于递归合并配置对象的函数。它处理了许多不同类型的值，特别是针对某些特定字段（如 alias, assetsInclude, noExternal, 和 plugins）进行了特殊的合并逻辑
 * @param defaults
 * @param overrides
 * @param rootPath
 */
...
/**
 * 作用是将两个配置对象进行合并。它支持递归地合并对象，但不能处理回调函数形式的配置
 * @param defaults 默认配置对象
 * @param overrides 覆盖配置对象，通常用于覆盖默认配置中的某些值
 * @param isRoot 一个可选参数，默认值为 true。用于标记当前是否是递归调用的根配置对象。在递归合并时，根对象的前缀会是空字符串，否则会加上 . 来标识嵌套的深层结构
 */
export function mergeConfig<
  D extends Record<string, any>, // D 和 O 是两个泛型类型，代表默认配置对象和覆盖配置对象。它们都必须是 Record<string, any> 类型，即键为字符串，值为任意类型的对象
...
>(
  ...
  // 递归地合并 defaults 和 overrides，并传递一个前缀（isRoot 为 true 时传空字符串，否则传 .）
...
/**
 * 函数的作用是将一个嵌套的异步数组（即包含异步操作的数组）展平，直到没有 Promise 对象为止，确保所有的异步任务都被处理完毕，并最终返回一个扁平化的数组
 * @param arr
 */
export async function asyncFlatten<T>(arr: T[]): Promise<T[]> {
  do {
    // do...while 循环用于反复处理数组中的 Promise 对象，直到数组中的所有元素都不再是 Promise。
    // 每次循环，都会等待数组中的所有 Promise 完成（await Promise.all(arr)），然后使用 flat(Infinity) 展开数组，直到所有嵌套层级都被展开
    arr = (await Promise.all(arr)).flat(Infinity) as any
    // 在每次循环中，arr.some((v: any) => v?.then) 会检查数组中是否还有 Promise 对象（即 then 方法），如果有，则继续循环展开。直到没有 Promise 对象为止
